...
info.update = function (props) {
    if(map.getZoom() >= mapZoom) {
        this._div.innerHTML = props ?
            '<b>Provinsi ' + props.NAME_1 + '<br />' + (props.NAME_2.startsWith('Kota') ? '' : 'Kota/Kabupaten ') + props.NAME_2 + '</b>' + '<br /> Prabowo-Hatta: ' + numberWithCommas(props.PRABOWO) + ', Jokowi-JK: ' + numberWithCommas(props.JOKOWI)
            : '<h4>Arahkan Kursor ke Kota/Kabupaten</h4>';
    }
    else {
        this._div.innerHTML = props ?
            '<b>Provinsi ' + props.NAME_1 + '<br /></b> Prabowo-Hatta: ' + numberWithCommas(props.PRABOWO) + ', Jokowi-JK: ' + numberWithCommas(props.JOKOWI)
            : '<h4>Arahkan Kursor ke Provinsi</h4>';
    }
};
...
function createTreeMap(data) {
    ...
    for (var i=0; i<dataKecamatan.length; ++i) {
        if (dataKecamatan[i]['Kota/Kabupaten'] == cityName) {
            var percentage = dataKecamatan[i]["Jokowi-JK"] / (dataKecamatan[i]["Jokowi-JK"] + dataKecamatan[i]["Prabowo-Hatta"]) * 100;
            districtList.push({
                name: dataKecamatan[i]["Tempat"],
    ...
    treeChart = new Highcharts.Chart({
        ...
        subtitle: {
            text: 'Tingkat Kecamatan untuk Provinsi ' + provinceName
        }
    });
...
function createPie(data) {
    ...
    if(map.getZoom() >= mapZoom) {
        // Zoom level: kabupaten
        var text = 'di Kota/Kabupaten ' + data.NAME_2;
    }
    else if(map.getZoom() >= mapZoomProvince) {
        // Zoom level: provinsi
        var text = 'di Provinsi ' + data.NAME_1;
    }
